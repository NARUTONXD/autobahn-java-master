...
import io.crossbar.autobahn.sdjx.adapter.MyContactsThreadListViewAdapter;
 ...
 * 部门联系人列表
 */
public class MyContactsPersonDetalActivity extends BaseActivity {
    ...
    //Model：部门的数据
    private List<ThreadContactBean.MsgBean.StaffBean> groups = new ArrayList<ThreadContactBean.MsgBean.StaffBean>();
    ...
    MyReceiver myReceiver = new MyReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (broadcastmsg.equals("{\"type\":\"whoislogintype\"}")) {
                if (getIntent().getStringExtra("mycontactstitleid1") != null) {
                    getData();
                } else {
                    getthread(getIntent().getStringExtra("personinfoid"));
                }
            }
        }
    };
        ...
        HttpManager.postAsync(BaseUrl + "/oainterface/Interfe/mail_department.html?", parmas,
                new HttpManager.ResultCallback<String>() {
                    ...
                    public void onResponse(String response) {
                        ThreadContactBean threadContactBean = gson.fromJson(response, ThreadContactBean.class);
                        content_tv_title.setText(threadContactBean.msg.department_info.dment_name);
                        groups.clear();
                        if (threadContactBean.code == 1) {
                            for (int i = 0; i < threadContactBean.msg.staff.size(); i++) {
                                groups.add(threadContactBean.msg.staff.get(i));
                            }
                            lv_contact.setAdapter(new MyContactsThreadListViewAdapter(groups, MyContactsPersonDetalActivity.this));
                            lv_contact.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                                @Override
                                public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                                    Intent intent = new Intent(MyContactsPersonDetalActivity.this, PersonInformationActivity.class);
                                    Person person = new Person(groups.get(i).organ_id,
                                            groups.get(i).organ_company,
                                            groups.get(i).organ_department,
                                            groups.get(i).organ_tdepartment,
                                            groups.get(i).organ_position,
                                            groups.get(i).organ_name,
                                            groups.get(i).organ_type,
                                            groups.get(i).organ_tel,
                                            groups.get(i).position_id,
                                            groups.get(i).position_levelid,
                                            groups.get(i).rank_id,
                                            groups.get(i).rank_level,
                                            groups.get(i).rank_name,
                                            groups.get(i).position_name,
                                            groups.get(i).organ_alias,
                                            groups.get(i).organ_email,
                                            groups.get(i).organ_famale,
                                            groups.get(i).account_state,
                                            groups.get(i).phone_isonline,
                                            groups.get(i).pad_isonline,
                                            groups.get(i).account_teldisplay,
                                            groups.get(i).departmentname,
                                            groups.get(i).organ_tdepartmentname
                                    );
                                    intent.putExtra("PersonInformationActivity", person);
                                    if (getIntent().getStringExtra("mycontactstitleid1") != null) {
                                        ...
                                        intent.putExtra("arr_position_id", groups.get(i).position_id);
...
//                lv_contact.setAdapter(adapter);
    ...
    public void getData() {
        ...
        if ("1".equals(infobytelBean.code)) {
            ...
            lv_contact.setAdapter(new SelectPersonAdapter(groups1, MyContactsPersonDetalActivity.this));
            lv_contact.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                @Override
                public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
